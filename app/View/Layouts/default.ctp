...
<head>
    ...
    <?php echo $this->Html->script('//malsup.github.com/jquery.form.js'); ?>
    <?php echo $this->Html->script('//ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/jquery-ui.min.js'); ?>
    <?php echo $this->Html->script('jquery.tablescroll'); ?>
    <?php echo $this->Html->css('jquery.tablescroll'); ?>
    <script>
        $(function() {
            setInterval(function() { ping(); }, 10000);
        })
        function ping() {
            var url = "<?=$this->Text->url(array('controller'=>'users','action'=>'ping.ajax'))?>";
            $.post(url);
        }
    </script>
</head>
